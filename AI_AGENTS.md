# AI Agent Context — markpdf.dev

## Summary

*one paragraph: what this site is, what it does*

markpdf.dev helps technical freelancers, consultants, and small dev shops create client-ready PDFs without leaving markdown. Users write proposals or SOWs in their editor, apply a saved brand template, preview the output, and export a polished PDF that looks professionally designed. The product sits closer to a dev tool than a design app: fast input, reusable config, predictable rendering, and clean PDF output.

## Audience

*one sentence: who this is for (broad demographic)*

Independent developers, technical consultants, DevOps freelancers, security consultants, and small software studios who send client documents regularly.
...
The ideal customer is a solo technical consultant or small dev agency that writes proposals, SOWs, audits, or project briefs in markdown but wastes time making them look professional in Canva, Google Docs, Notion, or PDF tools. They care about speed, polish, consistency, and repeatability. They are comfortable with markdown, value CLI/editor-friendly workflows, and will pay to remove formatting work from every client document.

## Goals

*1-2 sentences: primary business / product goal*

Launch with a narrow promise: markdown in, branded proposal PDF out. Win by making the exported PDF look premium enough that users trust it for real client work.
...
`proj=markpdf.dev`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=markpdf.dev` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=markpdf.dev` | Start the dev server, skipping install. |
| `make test proj=markpdf.dev` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=markpdf.dev
```
...
Create content around markdown proposal templates, SOW examples, freelancer proposal workflows, client-ready PDF generation, and comparisons against Google Docs, Canva, Notion, Pandoc, and Typst. Target developers searching for practical templates and workflow fixes, then convert them with free markdown templates and sample exports.

### Post-deploy checklist (do these once after the first successful deploy)

- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:markpdf.dev` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://markpdf.dev/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status markpdf.dev` resolves cleanly).
...
`portfolio project check markpdf.dev` (run from `sites/portfolio/`).
